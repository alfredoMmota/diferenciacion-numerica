    ...
    ui(new Ui::derivacion)
{
    ui->setupUi(this);
                    ...
                    //The connections of the interface buttons are made with the program
...
void derivacion::calcular(){//se define la funcion calcular
    ...
    //the variables that will be used in the program are declared with the values ​​that the user enters of double type
    ...
    H=ui->h->value();//se guardan los valores en la variables creada anteriormente
                     //the values ​​are stored in the variables created previously
    FXi=ui->fxi->value();
    FXi1=ui->fxi1->value();
    FXi2=ui->fxi2->value();
    FXi3=ui->fxi3->value();
    ...
    double primerderivada=-0.9125;//se definen los valores de las derivadas
                                  //the values ​​of the derivatives are defined
    ...
    //the result message is sent to the interface
...
void derivacion::borrar(){//se define la funcion borrar
    ...
    ui->fxi->setValue(0.0);//se rergesan todos los spinbox a 0.0
                           //All spinboxes are rerouted to 0.0
